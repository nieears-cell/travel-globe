...
EXT_CITIES: Dict[str, Tuple[float, float, str]] = {
    ...
    # 国内热门/常见目的地补充(cities.json 之外)
    "阿勒泰": (47.85, 88.14, "中国"), "伊宁": (43.92, 81.28, "中国"), "喀什": (39.47, 75.99, "中国"),
    "乌鲁木齐": (43.83, 87.62, "中国"), "景德镇": (29.27, 117.18, "中国"), "恩施": (30.27, 109.49, "中国"),
    "桂林": (25.27, 110.29, "中国"), "西双版纳": (22.01, 100.80, "中国"), "北海": (21.48, 109.12, "中国"),
    "延吉": (42.90, 129.51, "中国"), "淄博": (36.81, 118.05, "中国"), "天水": (34.58, 105.72, "中国"),
    "长白山": (42.02, 128.06, "中国"), "舟山": (29.99, 122.21, "中国"), "汕头": (23.35, 116.68, "中国"),
    "银川": (38.49, 106.23, "中国"), "西宁": (36.62, 101.78, "中国"), "兰州": (36.06, 103.83, "中国"),
    "太原": (37.87, 112.55, "中国"), "开封": (34.80, 114.31, "中国"), "大同": (40.08, 113.30, "中国"),
    "色达": (32.27, 100.33, "中国"), "茶卡盐湖": (36.70, 99.09, "中国"), "万宁": (18.80, 110.40, "中国"),
    "陵水": (18.51, 110.04, "中国"), "佛山": (23.02, 113.12, "中国"), "绍兴": (30.00, 120.58, "中国"),
    "温州": (28.00, 120.70, "中国"), "林芝": (29.65, 94.36, "中国"), "日喀则": (29.27, 88.88, "中国"),
    "漠河": (52.97, 122.54, "中国"), "满洲里": (49.60, 117.38, "中国"), "丹东": (40.13, 124.38, "中国"),
    "烟台": (37.46, 121.45, "中国"), "大连": (38.91, 121.61, "中国"), "秦皇岛": (39.94, 119.60, "中国"),
    "承德": (40.95, 117.94, "中国"),
}
...
def build_city_coords() -> Dict[str, Dict[str, object]]:
    ...
    # 显示规范化:region 桶 → 体面的地区名
    special = {"香港": "中国香港", "澳门": "中国澳门", "台北": "中国台湾", "高雄": "中国台湾",
               "垦丁": "中国台湾", "花莲": "中国台湾", "台南": "中国台湾", "台中": "中国台湾"}
    for nm, c in coords.items():
        if nm in special:
            c["country"] = special[nm]
        elif c.get("country") == "中国大陆":
            c["country"] = "中国"
    return coords
...
<body class="vintage">
<div id="boot" style="position:fixed;inset:0;display:flex;flex-direction:column;gap:18px;align-items:center;justify-content:center;background:#0a0d12;color:#cdbf9d;font:15px/1.6 Georgia,serif;z-index:99;letter-spacing:.22em;">
<style>@keyframes bootspin{from{transform:rotate(0)}to{transform:rotate(360deg)}}@keyframes bootbreathe{0%,100%{opacity:.45}50%{opacity:1}}</style>
<div style="width:52px;height:52px;border-radius:50%;border:1px solid rgba(214,169,87,.4);display:flex;align-items:center;justify-content:center;animation:bootspin 6s linear infinite;"><span style="color:#d6a957;font-size:22px;">✦</span></div>
<div style="animation:bootbreathe 2.2s ease-in-out infinite;">正在展开你的地球…</div>
</div>
<canvas id="gl"></canvas>
<div id="topbar">
...
var ITEMS = (function(){
  ...
  // "想去"心愿城市:虚线铭牌上球,不计入城数/里程/称号(复访钩子:勾掉一个就回来重生成)
  function withWish(been, wishArr){
    var w = citiesToItems((wishArr || []).filter(Boolean));
    var beenNames = {};
    been.forEach(function(it){ beenNames[it.place] = 1; });
    w = w.filter(function(it){ return !beenNames[it.place]; });
    w.forEach(function(it, i){ it.wish = true; it.id = been.length + i; });
    return been.concat(w);
  }
  try {
    var qs = new URLSearchParams(window.location.search);
    var q = qs.get('cities');
    if (q) {
      var it = citiesToItems(q.split(','));
      if (it.length) return withWish(it, (qs.get('wish') || '').split(','));
    }
    var raw = localStorage.getItem('myGlobeCities');
    if (raw) {
      var arr = JSON.parse(raw);
      if (arr && arr.length) {
        var it2 = citiesToItems(arr);
        var wr = [];
        try { wr = JSON.parse(localStorage.getItem('myGlobeWish') || '[]'); } catch (e2) {}
        if (it2.length) return withWish(it2, wr);
      }
    }
  } catch (e) {}
...
var BEEN = ITEMS.filter(function(it){ return !it.wish; });
var WISHES = ITEMS.filter(function(it){ return !!it.wish; });
...
// 账号钩子:?by=@你的账号 —— 烧进录屏水印与导出卡,图传到哪账号跟到哪(canvas文本,天然防注入)
var BY_HANDLE = (function(){ try { var b = new URLSearchParams(window.location.search).get('by'); return b ? String(b).slice(0, 24) : ''; } catch(e){ return ''; } })();
...
var HOME_PORT = (function(){
  ...
  var bn = ITEMS.filter(function(it){ return !it.wish; });
  if (bn.length && bn[0].custom) return { name: bn[0].place, lat: bn[0].lat, lon: bn[0].lon };
...
function distinctCountries(){ var s = {}; BEEN.forEach(function(it){ if (it.country) s[it.country] = 1; }); return Object.keys(s).length; }
document.getElementById('countLabel').textContent = IS_USER ? (BEEN.length + ' 座城市' + (WISHES.length ? ' · ☆' + WISHES.length : '')) : (ITEMS.length + ' photos');
document.getElementById('statusTitle').textContent = PAGE_TITLE;
document.getElementById('statusSub').textContent = IS_USER ? (BEEN.length + ' 城 · ' + distinctCountries() + ' 国/地区' + (WISHES.length ? ' · 心愿 ' + WISHES.length : '') + (IS_SHOWCASE ? ' · 示例' : '')) : ('0 groups / ' + ITEMS.length + ' photos');
...
(function(){
  ...
  if (IS_SHOWCASE) { cta.style.display = 'inline-block'; exp.style.display = 'inline-block'; exp.classList.add('mini'); exp.textContent = '⬇ 保存'; }
    ...
    try { edit.href = 'create.html?cities=' + encodeURIComponent(BEEN.map(function(it){ return it.place; }).join(',')) + (WISHES.length ? '&wish=' + encodeURIComponent(WISHES.map(function(it){ return it.place; }).join(',')) : ''); } catch(e){}
  }
...
if (BY_HANDLE) { try { document.getElementById('tourBrand').textContent = '✦ 我的旅行地球 · ' + BY_HANDLE; } catch(e){} }
...
nightMat.onBeforeCompile = function(shader){
  ...
  shader.fragmentShader = 'uniform float uTime;\nuniform vec3 uSunDir;\nvarying vec3 vWNormal;\n' +
    shader.fragmentShader.replace(
      ...
      '  float twBand = smoothstep(0.13, 0.012, abs(sunDot));\n' +
      '  twBand *= twBand;\n' +
      '  totalEmissiveRadiance += vec3(0.045, 0.115, 0.235) * dayF;\n' +
      '  totalEmissiveRadiance += vec3(0.72, 0.26, 0.08) * twBand * 0.20;\n' +
      '  float ember = smoothstep(0.012, -0.05, sunDot) * smoothstep(-0.16, -0.06, sunDot);\n' +
      '  totalEmissiveRadiance += vec3(0.30, 0.075, 0.02) * ember * 0.16;\n'
    );
...
// 夜景深空光晕:BackSide 大球 + fresnel 渐变,整体偏移制造主光源方向感(GitHub globe 做法,零后处理)
var haloMesh = new THREE.Mesh(
  new THREE.SphereGeometry(R * 1.24, 64, 48),
  new THREE.ShaderMaterial({
    uniforms: { uColor: { value: new THREE.Color(0x2f77c0) }, uPow: { value: 7.0 }, uAlpha: { value: .22 } },
    vertexShader: 'varying vec3 vN; void main(){ vN = normalize(normalMatrix * normal); gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }',
    fragmentShader: 'uniform vec3 uColor; uniform float uPow; uniform float uAlpha; varying vec3 vN; void main(){ float i = pow(clamp(0.72 - dot(vN, vec3(0.0, 0.0, 1.0)), 0.0, 1.0), uPow); gl_FragColor = vec4(uColor, i * uAlpha); }',
    side: THREE.BackSide, transparent: true, depthWrite: false, blending: THREE.AdditiveBlending
  })
);
haloMesh.position.set(.14, .16, -.06);
...
document.getElementById('resetView').onclick = function(){
  ...
  setStatus('视角已复位', IS_USER ? (BEEN.length + ' 座城市') : (ITEMS.length + ' 张照片'));
};
...
window.TravelGlobeTest = {
  ...
  exportJpeg: function(q){ return buildExportCanvas().toDataURL('image/jpeg', q || .82); },
  exportProbe: function(){
    ...
    return { w: o.width, h: o.height, home: HOME_PORT ? HOME_PORT.name : null, cities: BEEN.length, wishes: WISHES.length, index: voyageIndex(), badges: geoBadges(), countries: distinctCountries(), mileage: totalMileage(), avgLum: +(sum/samples).toFixed(1), brightFrac: +(bright/samples).toFixed(3) };
  }
};
...
function totalMileage(){
  var path = (HOME_PORT ? [HOME_PORT] : []).concat(BEEN);
  var sum = 0;
...
// 航海指数(制霸系分档计分:本命5 / 去过3 / 心愿1)——评论区攀比接龙的那个数字
function voyageIndex(){
  var been = BEEN.length, wish = WISHES.length;
  if (!been) return 0;
  return 5 + 3 * (been - 1) + wish;
}

// 地理成就(全部由坐标现算,零新数据;最多展示 2 枚)
function geoBadges(){
  if (!BEEN.length) return [];
  var lats = BEEN.map(function(it){ return it.lat; });
  var lons = BEEN.map(function(it){ return it.lon; });
  var minLat = Math.min.apply(null, lats), maxLat = Math.max.apply(null, lats);
  var minLon = Math.min.apply(null, lons), maxLon = Math.max.apply(null, lons);
  var out = [];
  if (minLat < 0 && maxLat > 0) out.push('跨越赤道');
  if (maxLat >= 55) out.push('北境行者');
  if (minLon < 0 && maxLon > 0) out.push('双半球足迹');
  if (maxLon - minLon >= 120) out.push('横跨三分之一个地球');
  if (distinctCountries() >= 5) out.push('护照旅人');
  if (BEEN.some(function(it){ return Math.abs(it.lat) <= 23.5; })) out.push('热带航线');
...
function personaFromCities(){
  var tagged = [];
  BEEN.forEach(function(it){ var t = CITY_TAGS[it.place]; if (t) tagged.push(t); });
  var n = tagged.length;
  if (n < 3) return null;
...
function buildExportCanvas(){
  ...
  ctx.fillStyle = fade; ctx.fillRect(gX, gTop + gW - 300, gW, 340);
  ...
  [[36,36,1,1],[W-36,36,-1,1],[36,H-36,1,-1],[W-36,H-36,-1,-1]].forEach(function(c){
    ctx.beginPath();
    ctx.moveTo(c[0] + 34*c[2], c[1]); ctx.lineTo(c[0], c[1]); ctx.lineTo(c[0], c[1] + 34*c[3]);
    ctx.stroke();
  });
  ...
  ctx.fillText(BEEN.length + ' 城 · ' + distinctCountries() + ' 国/地区', W/2, 1512);
  var totalCities = Object.keys(CITY_COORDS).length;
  var pct = Math.round(BEEN.length / totalCities * 100);
  var m = totalMileage(); var laps = m / 40075;
  var fv = farthestVoyage();
  var cells = [
    ['世界城市图鉴', '已点亮 ' + pct + '%'],
    ['旅行足迹', '超越约 ' + travelPercentile(BEEN.length) + '% 的人 *'],
  ...
  var idxTxt = '航海指数 ' + voyageIndex();
  var bds = geoBadges();
  if (WISHES.length) idxTxt += ' · 心愿 ' + WISHES.length + ' 城';
  bds.forEach(function(b){ idxTxt += ' · ' + b; });
  ctx.fillStyle = gold; ctx.font = '600 27px ' + zhFont;
  ctx.fillText(idxTxt, W/2, 1798);
  ctx.fillStyle = faint; ctx.font = '400 20px ' + zhFont;
  ctx.textAlign = 'left';
...
function updateIntro(now){
  ...
  if (elapsed >= intro.total) {
    ...
    targetRadius = radius;
    completeIntro(false);
    setStatus(IS_USER ? '轻转地球' : 'Idle orbit', IS_USER ? (BEEN.length + ' 城 · ' + distinctCountries() + ' 国/地区') : (ITEMS.length + ' photos ready'));
  }
}
...
function updateIntroPhotos(elapsed){
  photoGroups.forEach(function(group){
    ...
    if (eased > .96 && !group.userData.rippled){ group.userData.rippled = true; var itR = group.userData.item; spawnRipple(itR.lat, itR.lon); }
...
function buildRoutes(){
  ...
  // 航线从母港起航,再串起各张照片
  var beenNodes = ITEMS.filter(function(it){ return !it.wish; });
  var nodes = HOME_PORT ? [HOME_PORT].concat(beenNodes) : beenNodes;
...
// 彗星头飞线:aProgress + fract(uTime) 沿整条航线循环跑一个发光头(deck.gl 思路的零依赖版)
var cometLine = null;
function buildComet(pts){
  if (!pts || pts.length < 2) return;
  if (cometLine){ scene.remove(cometLine); }
  var n = pts.length;
  var pos = new Float32Array(n * 3), prog = new Float32Array(n);
  pts.forEach(function(v, i){
    var vv = v.clone().multiplyScalar(1.004);
  ...
  var mat = new THREE.ShaderMaterial({
    uniforms: { uTime: { value: 0 }, uColor: { value: new THREE.Color(0xffd27a) } },
    vertexShader: 'attribute float aP; varying float vP; void main(){ vP = aP; gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }',
    fragmentShader: 'uniform float uTime; uniform vec3 uColor; varying float vP; void main(){ float head = fract(uTime * 0.045); float d = fract(head - vP); float a = smoothstep(0.11, 0.0, d); gl_FragColor = vec4(uColor, a * 0.85); }',
    transparent: true, depthWrite: false, blending: THREE.AdditiveBlending
  });
  cometLine = new THREE.Line(geo, mat);
  cometLine.frustumCulled = false;
  scene.add(cometLine);
}

// 落钉涟漪:贴球面的圆环,scale 张开 + alpha 衰减(three-globe rings 层的轻量版)
var ripples = [];
function spawnRipple(lat, lon){
  var pos = latLonToVec(lat, lon, R + .015);
  var geo = new THREE.RingGeometry(.82, .96, 48);
  var mat = new THREE.MeshBasicMaterial({
    color: THEMES[currentTheme].glow, transparent: true, opacity: .85,
    side: THREE.DoubleSide, depthWrite: false, blending: THREE.AdditiveBlending
  });
  var m = new THREE.Mesh(geo, mat);
  m.position.copy(pos);
  m.lookAt(pos.clone().multiplyScalar(2));
  m.scale.setScalar(.06);
  m.userData.born = performance.now();
  scene.add(m);
  ripples.push(m);
  if (ripples.length > 24){ var old = ripples.shift(); scene.remove(old); }
}
function updateRipples(now){
  for (var i = ripples.length - 1; i >= 0; i--){
    var m = ripples[i];
    var t = (now - m.userData.born) / 950;
    if (t >= 1){ scene.remove(m); ripples.splice(i, 1); continue; }
    m.scale.setScalar(.06 + t * .34);
    m.material.opacity = (1 - t) * .85;
  }
}
...
function paintNameplate(group){
  ...
  ctx.fill();
  ctx.restore();
  var isWish = !!item.wish;
  if (isWish){ ctx.save(); ctx.setLineDash([11, 8]); ctx.strokeStyle = theme.pinBorder; ctx.lineWidth = 2.4; ctx.stroke(); ctx.restore(); }
  else { ctx.strokeStyle = theme.pinBorder; ctx.lineWidth = 2; ctx.stroke(); }
  ...
  // 本命城市徽章 / 想去标记
  var isHome = !isWish && (typeof HOME_PORT !== 'undefined' && HOME_PORT && item.place === HOME_PORT.name);
  if (isHome) {
    ctx.textAlign = 'center';
    ...
    ctx.textAlign = 'center';
    ctx.fillStyle = nightMode ? 'rgba(150,215,255,.85)' : (dataMode ? 'rgba(126,247,212,.8)' : 'rgba(74,104,138,.85)');
    ctx.font = glassMode ? '700 20px Segoe UI, Arial' : '700 20px Georgia, serif';
    ctx.fillText('☆ 想去', 192, 80);
  }
  // 城市名(主角)
  ctx.textAlign = 'center';
  ctx.fillStyle = isWish ? (nightMode ? 'rgba(223,241,255,.72)' : (dataMode ? 'rgba(167,240,230,.72)' : 'rgba(60,33,22,.62)')) : theme.pinText;
  var name = item.place || '';
  ...
  if (currentTheme === 'vintage') {
    ...
    // 朱印:本命城=「母港」,想去=「心愿」(靛蓝),其余=「行迹」
    ctx.save();
    ctx.translate(322, 428); ctx.rotate(-.06);
    ctx.fillStyle = isWish ? 'rgba(58,92,128,.78)' : 'rgba(172,48,36,.8)';
    roundRect(ctx, -24, -24, 48, 48, 7); ctx.fill();
    ctx.fillStyle = 'rgba(246,238,220,.94)';
    ctx.font = '700 19px "Microsoft YaHei", serif';
    ctx.textAlign = 'center';
    ctx.fillText(isWish ? '心愿' : (isHome ? '母港' : '行迹'), 0, 7);
    ctx.restore();
    ctx.textAlign = 'center';
  }
...
function switchTheme(key){
  ...
  haloMesh.visible = key === 'night';
  if (cometLine){ cometLine.visible = key !== 'data'; cometLine.material.uniforms.uColor.value.setHex(key === 'night' ? 0x8ce0ff : 0xffd27a); }
  anchorLines.forEach(function(line){
...
function frame(now){
  ...
  if (cometLine && cometLine.visible) cometLine.material.uniforms.uTime.value = now * .001;
  updateRipples(now);
